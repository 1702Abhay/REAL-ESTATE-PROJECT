...
def csv_add(file):
    ...
    with open(file, "r") as f:
        for line in f:
            ...
            dataset = line.split(",")
            if dataset[0]!="owner":
                cursor.execute('INSERT INTO propertydata (owner,district,taluk,type,age,mobile,bedrooms,sqft,description,price) VALUES (%s, %s,%s, %s,%s, %s,%s, %s,%s, %s)',(dataset[0],dataset[1],dataset[2],dataset[3],dataset[4],dataset[5],dataset[6],dataset[7],dataset[8],dataset[9]))
...
def search_data():
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute("SELECT DISTINCT taluk FROM propertydata")
